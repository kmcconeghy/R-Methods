...
## Tie everything together  
  ...
  Now that the methods are established, a panel dataset can be constructed where each row is a unique group, year, month. The primary measure will be the incidence density.  
...
```{r MakePanel}
  ...
  #Build Panel
    dfPanel <- unique(as_tibble(expand.grid(groups,years,months)))
    names(dfPanel) <- c("Group","Year","Month")
    dfPanel <- dfPanel %>% arrange(Group, Year, Month)
  
  #Build Matching TimeVars
   df <- df %>%  
     mutate(YearIn = year(CohortEntry)) %>%
     mutate(YearOut = year(CohortExit)) %>%
     mutate(MonthIn = month(CohortEntry)) %>%
     mutate(MonthOut = month(CohortExit))
       
  #A full function that incorporates all steps
    MakePanel <- function(Panel, dfid) {
      Panel <- c(Panel[[1]],as.integer(Panel[[2]]), as.integer(Panel[[3]]))
      dfidpanel <- df %>%
        filter(State == Panel[[1]],
               YearIn <= Panel[[2]], YearOut >=Panel[[2]],
               MonthIn<= Panel[[3]], MonthOut>=Panel[[3]])
      CountRes <- nrow(dfidpanel)
      return(CountRes)
    }
  dfPanel$Residents <- apply(dfPanel, 1, function(x) MakePanel(x, df))
  ...
  kable(head(dfPanel, n=12), align=c('c'), digits=4)  
```
