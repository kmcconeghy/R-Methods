...
```{r funcWhite, echo=F}
vce <- c("HC0","HC1","HC2","HC3","HC4")
for (i in 1:5) {
  hc <- coeftest(m1, vcovHC(m1, type=vce[i])) #Standard
  i <- i+3
  se_results[i,3] <- hc[2,1]
  se_results[i,4] <- hc[2,2]
  se_results[i,5] <- hc[3,1]
  se_results[i,6] <- hc[3,2]
}
